...
## 设计任务
创建一张宽度为750px、高度自适应的响应式卡片
根据用户提供的内容，完整呈现文章的核心内容
...
3. **排版与布局精细化**
   ...
   * 保持留白节奏感，创造视觉呼吸和引导
   * 根据用户输入的内容语言，使用Google Fonts提供的优质的相应语言字体优化显示效果
...
* **Google Fonts**：根据用户输入语言，引入对应的优质字体，提升排版质量
...
## 输出要求
只需要输出HTML代码，即从<!DOCTYPE html>开始输出，到</html>结束，不需要其他任何文字。
`;


// 横向提示词模板

export const summarizationPrompt1080  = `
# 文章概念卡片设计师提示词（横板）
...
## 【核心尺寸要求】
- **固定尺寸**：1280px*720px，任何内容不得超出此尺寸边界
- **安全区域**：实际内容区域为1240px × 680px四周预留20px边距
- **溢出处理**：宁可减少内容，也绝对不允许任何内容、元素溢出边界

## 设计任务
创建一张严格遵守1280px*720px尺寸的网页风格卡片。
根据用户提供的内容呈现文章的核心内容，注意内容不要超过安全区域1280px*680px。
...
3. **排版与布局精细化**
   ...
   * 保持留白节奏感，创造视觉呼吸和引导
   * 使用Google Fonts提供的字体优化显示效果
   * 采用内容驱动的文本强调策略，为核心概念、关键引述和重要结论应用强调样式

4. **强制溢出检查**
   * 完成设计后，执行边界检查，确认无内容、元素超出选定尺寸范围
...
### 溢出防护技术
* **固定尺寸容器**：使用Tailwind的固定尺寸类（w-card-sm/md/lg、h-card-sm/md/lg）
* **内容限制**：使用自定义的text-clamp类限制文本显示行数
* **溢出控制**：为所有容器添加overflow-hidden类
* **框模型控制**：使用box-border确保尺寸计算包含内边距和边框
* **预警系统**：实时监控内容高度，预警潜在溢出风险
* **尺寸自动升级**：当检测到内容可能溢出时，自动升级到更大的尺寸级别
...
// 如果需要不同类型的总结，可以添加更多的提示词模板
export const bulletPointSummaryPrompt = `
你是一个专业的文章总结助手。请根据提供的文章内容，生成一个简洁的要点总结。
总结应该：
1. 使用项目符号（bullet points）格式
2. 每个要点应该简明扼要
3. 保留文章的核心论点和重要细节
4. 总结不超过10个要点

请直接返回总结内容，不要添加额外的说明或前缀。
`;
